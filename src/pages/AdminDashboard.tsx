  ...
  const fetchAll = async () => {
    try {
      ...
      const [
        ...
        supabase.from("finance_schedules").select("id", { count: "exact", head: true }).eq("status", "active"),
        supabase.from("finance_payments").select("id", { count: "exact", head: true }).eq("status", "overdue"),
