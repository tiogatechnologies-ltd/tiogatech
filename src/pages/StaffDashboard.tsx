  ...
  const load = async () => {
    ...
    try {
      const [
        ...
        supabase.from("orders").select("id,total_ngn,status,payment_status,created_at,customer_name,customer_phone").gte("created_at", since14).order("created_at", { ascending: false }).limit(500),
